...
// In Cloudflare Pages: Settings → Environment Variables → add each VITE_ key.
...
const dbService = {
  getCollectionRef: () => collection(db, 'jobs'),

  addJob: async (jobData) => {
    try {
      await addDoc(dbService.getCollectionRef(), jobData);
    } catch (e) {
  ...
  deleteJob: async (id) => {
    try {
      await deleteDoc(doc(dbService.getCollectionRef(), id));
    } catch (e) {
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h2 className="text-xl font-bold text-zinc-100 tracking-tight">{job.title}</h2>
            {isNew(job.created) && (
...
const JobForm = ({ onSave, onCancel }) => {
  const [formData, setFormData] = useState({
    title: '', company: '', location: '', city: '', state: 'Select',
    payrangemin: '', payrangemax: '', paytype: 'Pay Type Not Specified',
    category: 'Full-time', content: '', contact: '',
  });
  ...
  const handleAIParsing = () => {
    ...
    setTimeout(() => {
      setFormData({
        title: 'Trauma Scene Cleaner', company: 'Bio-Recovery Experts',
        location: '33101', city: 'Miami', state: 'FL',
        payrangemin: 28, payrangemax: 40, paytype: 'Hourly',
        category: 'Full-time', content: aiText || 'Standard biohazard cleanup duties...', contact: 'apply@biorecovery.mock',
      });
  ...
  if (step === 3) {
    return (
      <div className="max-w-md mx-auto bg-zinc-900 p-8 rounded-xl border border-zinc-800 text-center shadow-2xl mt-8">
        ...
        <p className="text-zinc-400 mb-6 text-sm font-mono">Secure your position on the board. Flat rate $99.</p>
        ...
        >
          {isProcessing ? 'Authorizing...' : 'Authorize $99.00'}
        </button>
  ...
  return (
    <div className="max-w-3xl mx-auto bg-zinc-900 p-8 rounded-xl border border-zinc-800 shadow-2xl mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div>
          ...
          <select value={formData.state} onChange={e => setFormData({ ...formData, state: e.target.value })} className={inputClass}>
            <option>Select</option><option value="CA">CA</option><option value="FL">FL</option><option value="NV">NV</option><option value="NY">NY</option><option value="TX">TX</option>
          </select>
        </div>
      </div>
      ...
      <div className="flex justify-end space-x-4 pt-6 border-t border-zinc-800">
        ...
        <button onClick={() => setStep(3)} className="px-8 py-2.5 bg-amber-500 text-zinc-950 font-bold uppercase tracking-wide rounded-md hover:bg-amber-400 transition-colors active:scale-[0.98]">Proceed to Checkout</button>
      </div>
    </div>
  );
};
...
// --- ADMIN DASHBOARD ---
const AdminDashboard = ({ jobs, onExit, onShowMessage }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      step++;
      if (step === 1) setAgentLogs(prev => [...prev, '[AGENT] Waking up. Initiating daily web scrape...']);
      if (step === 2) setAgentLogs(prev => [...prev, '[AGENT] Found 14 raw URLs. Extracting text content...']);
      if (step === 3) setAgentLogs(prev => [...prev, '[AGENT] Passing raw data to GPT-4o for structural mapping...']);
      if (step === 4) {
        setAgentLogs(prev => [...prev, '[AGENT] Formatting complete. ✅ Deploying 1 high-confidence record.']);
        await dbService.addJob({
          title: 'Hazardous Materials Removal Worker',
          company: 'Enviro-Clean Response',
          location: '89101', city: 'Las Vegas', state: 'NV',
          payrangemin: 35, payrangemax: 55, paytype: 'Hourly',
          category: 'Full-time',
          content: '[AUTOMATED IMPORT]\n\nImmediate need for HAZMAT certified workers in the Las Vegas area. Duties include chemical spill cleanup, biohazard removal, and sanitization of contaminated zones.',
          contact: 'hr@enviroclean-nv.mock',
          created: new Date().toISOString(),
        });
      }
      if (step === 5) {
        setAgentLogs(prev => [...prev, '[SYSTEM] Operation complete. Returning to sleep for 24h.']);
        setAutoPilot(false);
      }
    }, 2500);
  ...
  const runSimulationScan = () => {
    ...
    setTimeout(() => {
      setScrapedJobs([{
        id: `scraped-${Date.now()}`,
        title: 'Emergency Response Cleaner',
        company: `PuroClean ${scanLocation}`,
        location: '75001',
        city: scanLocation === 'Nationwide' ? 'Dallas' : scanLocation,
        state: 'TX',
        payrangemin: 22, payrangemax: 28, paytype: 'Hourly',
        category: 'Full-time',
        content: 'URGENT HIRE: Seeking technicians for immediate deployment. Strict OSHA protocols required.',
        contact: 'careers@puroclean.mock',
        created: new Date().toISOString(),
      }]);
      setIsScanning(false);
    }, 2000);
  ...
  const approveJob = async (jobToImport) => {
    const { id, ...jobData } = jobToImport;
    await dbService.addJob(jobData);
    setScrapedJobs(prev => prev.filter(j => j.id !== id));
    onShowMessage('Import Successful', `Added ${jobData.title} to live database.`, 'info');
  };

  const rejectJob = (id) => setScrapedJobs(prev => prev.filter(j => j.id !== id));
          ...
          <div className="space-y-8">
            <div className="bg-zinc-950 border border-amber-500/30 p-6 rounded-lg relative overflow-hidden shadow-[0_0_15px_rgba(245,158,11,0.05)]">
              ...
              <p className="text-xs text-zinc-400 mb-4 max-w-xl leading-relaxed font-mono">
                When active, the AI Agent wakes every 24 hours, scrapes regional dispatch boards, formats data, and deploys records to the live database.
              </p>
              <div
                  ...
                  {scrapedJobs.map(job => (
                    <div key={job.id} className="bg-zinc-950 border border-zinc-800 p-5 rounded-lg flex justify-between items-center group hover:border-amber-500/30 transition-colors">
                      <div>
                        ...
                        <p className="text-sm text-zinc-400">{job.company} • {job.city}, {job.state}</p>
                      </div>
          ...
          <div className="space-y-2" role="tabpanel">
            ...
            {jobs.map(job => (
              <div key={job.id} className="bg-zinc-950 border border-zinc-800 p-4 rounded-lg flex justify-between items-center">
                ...
                <button
                  onClick={() => dbService.deleteJob(job.id)}
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    return onSnapshot(dbService.getCollectionRef(), (snapshot) => {
  ...
  const handleAddJob = async (newJob) => {
    ...
    setCurrentView('home');
    showMessage('Success', 'Listing deployed successfully.', 'info');
  };
  ...
  const requestDeleteJob = (id) => {
    setModal({
      ...
      type: 'danger',
      onConfirm: async () => { await dbService.deleteJob(id); setModal(prev => ({ ...prev, isOpen: false })); },
        ...
        <nav className="sticky top-0 z-50 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800" aria-label="Main Navigation">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <button
              ...
              className="bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-100 px-5 py-2 rounded font-bold text-sm uppercase tracking-wide transition flex items-center"
            >
              <PlusCircle className="w-4 h-4 mr-2 text-amber-500" aria-hidden="true" /> Post a Job
            </button>
          </div>
        </nav>
      </header>
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        {currentView === 'login' ? (
          <div className="max-w-md mx-auto mt-20 bg-zinc-900 p-8 border border-zinc-800 rounded-xl shadow-2xl text-center">
            ...
            <button
              onClick={() => { setIsAdmin(true); setCurrentView('admin'); }}
          ...
          <AdminDashboard jobs={jobs} onShowMessage={showMessage} onExit={() => { setIsAdmin(false); setCurrentView('home'); }} />
        ) : currentView === 'post' ? (
          <JobForm onSave={handleAddJob} onCancel={() => setCurrentView('home')} />
        ) : (
          <>
